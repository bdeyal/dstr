...
//
class DString;
class DStringMatchVector;
//-----------------------------------------------

// A "View" of a char* and length
//
class DStringView {
public:
...
public:
    ...
    typedef DStringMatchVector MatchVector;
...
#if !defined(NO_DSTRING_REGEX)
// STL style, read only wrapper around DSTR_Match_Vector
//
class DStringMatchVector {
public:
    DStringMatchVector()  { init(); }
    ~DStringMatchVector() { clear(); }

    const DSTR_Regex_Match& operator[](size_t index) const {
        check(index);
        return mvec.matches[index]; }
    size_t size() const { return mvec.matches_len; }

    typedef DSTR_Regex_Match        value_type;
    typedef DSTR_Regex_Match&       reference;
    typedef DSTR_Regex_Match&       const_reference;
    typedef DSTR_Regex_Match*       iterator;
    typedef const DSTR_Regex_Match* const_iterator;
    typedef ptrdiff_t               difference_type;
    typedef size_t                  size_type;
...
private:
    ...
    friend class DString;
    friend class DStringView;

    // Initialized mvec sent to C API. Not checked
    //
    DSTR_Match_Vector* self() {
        clear(); // otherwise leak on reuse
        return &mvec;
    }
    void init()  {
        mvec.matches_len = 0;
    ...
    void check(size_t index = 0) const {
        if (mvec.matches == nullptr || index >= mvec.matches_len) {
            throw DStringError("MatchVector::check()");  }
    }
};
#endif
/*-------------------------------------------------------------------------------*/
